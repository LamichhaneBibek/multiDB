...
// SQLiteConnection represents the SQLite database connection.
type SQLiteConnection struct {
	Config *DBConfig
}

func (conn *SQLiteConnection) Connect() (DBConnection, error) {
	db, err := gorm.Open(sqlite.Open(conn.Config.DB), &gorm.Config{})
	return db, err
}
